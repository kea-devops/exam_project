...
def index(request):
    ...
    accounts = Account.objects.filter(
        customerid=customer).select_related('account_typeid')
    ...
    if request.method == 'POST':
        ...
        if account_form.is_valid():
            account_type = get_object_or_404(
                Account_type, name=request.POST['account_type'])
...
# def transfer_money(request, pk):
 #  customer = get_object_or_404(Customer, pk=pk)
  # accounts = Account.objects.filter(customerid=customer).select_related('account')
   # context = {'customer': customer, 'accounts': accounts}
